...
function VRMModel({ url, controls }) {
  ...
  useFrame(() => {
    ...
    // Mouth visemes with smoothing
    Object.keys(r.mouth).forEach((name) => {
      const key = name.split("_MTH_")[1]?.toUpperCase();
      const target = s.mouth[key] ?? 0;
      const current = blendMapRef.current[name][0].mesh.morphTargetInfluences[0];
      setBlend(name, lerp(current, target, 0.35));
    });
  ...
  return (
    <div style={{ display: "flex", height: "100vh", position: "relative" }}>
      ...
      >
        ...
        <h3>Body Controls</h3>
        {["X","Y","Z"].map(axis => createSlider(`Head ${axis}`, -0.5, 0.5, 0.01, controls[`head${axis}`], e => updateControls({...controls, [`head${axis}`]: parseFloat(e.target.value)})))}
        {["X","Y","Z"].map(axis => createSlider(`Spine ${axis}`, -0.5, 0.5, 0.01, controls[`spine${axis}`], e => updateControls({...controls, [`spine${axis}`]: parseFloat(e.target.value)})))}
        {["X","Y","Z"].map(axis => createSlider(`Left Arm ${axis}`, -1, 1, 0.01, controls[`leftArm${axis}`], e => updateControls({...controls, [`leftArm${axis}`]: parseFloat(e.target.value)})))}
        {["X","Y","Z"].map(axis => createSlider(`Right Arm ${axis}`, -1, 1, 0.01, controls[`rightArm${axis}`], e => updateControls({...controls, [`rightArm${axis}`]: parseFloat(e.target.value)})))}
        {["X","Y","Z"].map(axis => createSlider(`Left Leg ${axis}`, -1, 1, 0.01, controls[`leftLeg${axis}`], e => updateControls({...controls, [`leftLeg${axis}`]: parseFloat(e.target.value)})))}
        {["X","Y","Z"].map(axis => createSlider(`Right Leg ${axis}`, -1, 1, 0.01, controls[`rightLeg${axis}`], e => updateControls({...controls, [`rightLeg${axis}`]: parseFloat(e.target.value)})))}

        <h3>Blink</h3>
        <label style={{ display:"flex", alignItems:"center", margin:"2px 0" }}>
          <input type="checkbox" checked={controls.blinkToggle} onChange={e=>updateControls({...controls, blinkToggle: e.target.checked})} />
          <span style={{marginLeft:"8px"}}>Blink Toggle</span>
        </label>
        {createSlider("Blink Intensity", 0,1,0.01, controls.blinkIntensity, e=>updateControls({...controls, blinkIntensity: parseFloat(e.target.value)}))}

        <h3>Emotions</h3>
        {createSlider("Emotion Intensity", 0,1,0.01, controls.emotionIntensity, e=>updateControls({...controls, emotionIntensity: parseFloat(e.target.value)}))}
        {Object.keys(controls.emotions).map(key=>(
          <label key={key} style={{display:"flex", alignItems:"center"}}>
            <input type="checkbox" checked={controls.emotions[key]} onChange={e=>updateControls({...controls, emotions:{...controls.emotions, [key]: e.target.checked}})} />
        ...
        {Object.keys(controls.mouth).map(key =>
          createSlider(key,0,1,0.01,controls.mouth[key], e=>updateControls({...controls, mouth:{...controls.mouth, [key]:parseFloat(e.target.value)}}))
        )}
      </div>
